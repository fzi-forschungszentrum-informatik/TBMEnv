...
OPTIMIZED_VAR_VALUES_PATH = base_path / "baseline1_optimized_variables.json"
...
class Baseline1:
    ...
    def predict(
        ...
        use_tuned_variables=True,
        rerun_tune_variables=False,
    ):
        """
        ...
        use_kalman_filter : bool
            If `True` will use `estimate_initial_state_kalman_filter`.
            Else will use `estimate_initial_state_extrapolate_obs` instead.
        use_tuned_variables : bool
            If `True` will use tuned parameters of the thermal model.
            If `False` will use the default parameters, which show
            much worse performance. See the paper for details.
        rerun_tune_variables : bool
            If set to `True` will compute model variables from scratch.
            Else will use precomputed values instead. Running from scratch
            will take roughly 10h with 40 cores.
            Only relevant if `use_tuned_variables`. I
        ...
        """
        if use_tuned_variables and self.n_predictions % 672 == 0:
            # JSON must have strings as keys.
            tune_run_id = str(self.n_predictions)

            if OPTIMIZED_VAR_VALUES_PATH.is_file():
                with OPTIMIZED_VAR_VALUES_PATH.open("r") as f:
                    optimized_var_values = json.load(f)
            else:
                optimized_var_values = {}

            if rerun_tune_variables or tune_run_id not in optimized_var_values:
                best_config = self.tune_variables()

                optimized_var_values[tune_run_id] = best_config
                with OPTIMIZED_VAR_VALUES_PATH.open("w") as f:
                    json.dump(optimized_var_values, f, indent=2)

            else:
                best_config = optimized_var_values[tune_run_id]
    ...
    def evaluate_conifg(
        ...
        losses = {"mae_loss": mae_loss, "rmse_loss": rmse_loss}
        if return_losses:
            return losses
        else:
            train.report(losses)
